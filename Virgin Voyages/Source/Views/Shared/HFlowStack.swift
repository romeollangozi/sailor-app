...
struct HFlowStack: Layout {
	var alignment: Alignment
	var horizontalSpacing: CGFloat?
	var verticalSpacing: CGFloat?
	var fitContentWidth: Bool

	@inlinable init(alignment: Alignment = .center,  horizontalSpacing: CGFloat? = nil, verticalSpacing: CGFloat? = nil, fitContentWidth: Bool = false) {
...
extension HFlowStack {
	...
	private func arrangeRows(proposal: ProposedViewSize, subviews: Subviews, cache: inout Cache) -> [Row] {
		if subviews.isEmpty {
			return []
		}
		...
		for index in rows.indices {
			let maxHeightIndex = rows[index].elements.max {
