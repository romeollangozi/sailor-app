//
//  GetHealthCheckDetailRequest.swift
//  Virgin Voyages
//
...
struct GetHealthCheckDetailRequest: AuthenticatedHTTPRequestProtocol {
    
    let input: GetHealthCheckDetailDataInput
    
    var path: String {
        return NetworkServiceEndpoint.healthCheckDetail
    }
...
struct GetHealthCheckDetailDataInput {
    let reservationGuestId: String
    let reservationId: String
}

struct GetHealthCheckDetailResponse: Decodable {
    ...
    struct HealthCheckRefusePage: Decodable {
        let imageURL: String?
        let description: String?
        let title: String?
    }
    
    struct HealthCheckReviewPage: Decodable {
        let imageURL: String?
        let title: String?
        let description: String?
        let confirmationQuestion: String?
...
extension NetworkServiceProtocol {
    ...
    func getHealthCheckDetail(reservationGuestId: String,
                              reservationId: String) async throws -> GetHealthCheckDetailResponse? {
        
        let request = GetHealthCheckDetailRequest(input: GetHealthCheckDetailDataInput(reservationGuestId: reservationGuestId,
                                                                                       reservationId: reservationId))
        
        return try await self.requestV2(request, responseModel: GetHealthCheckDetailResponse.self)
    }
}
...
extension GetHealthCheckDetailResponse {
    ...
    static func mock() -> GetHealthCheckDetailResponse {
        
        return GetHealthCheckDetailResponse(
            ...
            landingPage: LandingPage(
                ...
                questionsPage: LandingPage.QuestionsPage(
                    healthQuestions: [
                        .init(
                            title: "Question 1",
                    ...
                    """,
                    travelParty: LandingPage.QuestionsPage.TravelParty(
                        title: "Cabin mates",
            ...
            healthCheckRefusePage: HealthCheckRefusePage(
            ...
            healthCheckReviewPage: HealthCheckReviewPage(
                imageURL: "",
